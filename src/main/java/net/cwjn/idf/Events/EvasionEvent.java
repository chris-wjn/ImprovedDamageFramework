...
public class EvasionEvent {
    ...
    public static void Evasion(LivingAttackEvent event) {
        ...
        if (target instanceof Player) {
            if (target.getAttributeValue(AttributeRegistry.EVASION.get())/100 >= Math.random()) {
                IDFPackerHandler.INSTANCE.send(PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(target.getX(), target.getY(), target.getZ(), 30d, target.getCommandSenderWorld().dimension())),
